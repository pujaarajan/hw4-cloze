...
def main(options):
  ...
  optimizer = eval("torch.optim." + options.optimizer)(lstmlm.parameters(), options.learning_rate)
  ...
  for epoch_i in range(options.epochs):
    ...
    for i, batch_i in enumerate(utils.rand.srange(len(batched_train_in))):
      ...
      sys_out_batch = lstmlm(train_in_batch)  # (seq_len, batch_size, vocab_size) # TODO: substitute this with your module
    ...
    for batch_i in range(len(batched_dev_in)):
      ...
      sys_out_batch = lstmlm(dev_in_batch)
    ...
    if (last_dev_avg_loss - dev_avg_loss).data[0] < options.estop:
      ...
      break
    torch.save(lstmlm, open(options.model_file + ".nll_{0:.2f}.epoch_{1}".format(dev_avg_loss.data[0], epoch_i), 'wb'), pickle_module=dill)
