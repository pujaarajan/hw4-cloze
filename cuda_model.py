...
class BiRNNLMwithDropout(nn.Module):
    ...
    def forward(self, input_batch, withDropout = True):
        ...
        predictions = Variable(torch.zeros(seq_len, batch_size, self.vocab_size), requires_grad=False).cuda()
        hLR = Variable(torch.rand(seq_len + 1, batch_size, self.hidden_size), requires_grad=False).cuda()
        hRL = Variable(torch.rand(seq_len + 1, batch_size, self.hidden_size), requires_grad=False).cuda()
